...
const string VERSION = "Warfarin 解释器 2.30";
...
#define RUN_ERR(msg) \
    do {\
        ...
        cout << "当前行：" << currentCommand <<endl;\
        throw ExecuteException();\
    } while(0)

#define STOP_EXEC(msg) \
    do {\
        cout << "检测到强制退出: " << msg << endl;\
        throw DenyExecute();\
    } while(0)
...
void checkHandle(int handle) {
	if(handle < 0 || handle >= filePtr.size()) RUN_ERR("无效句柄: " + parseNum(handle));
}
...
struct Value {
    ...
    Value operator + (Value & v) {
        if(type == T_STRING || v.type == T_STRING) return Value::makeString(strVal + v.toString());
            ...
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 相加。");
            return Value::makeNumber(v.numVal + numVal);
        } else if(type == T_REF) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Ref 相加。");
            return Value::makeRef(v.numVal + refVal);
        } else RUN_ERR("类型 " + typeName[type] + " 不支持 '+'.");
    }

    Value operator - (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 相减。");
            return Value::makeNumber(numVal - v.numVal);
        } else if(type == T_REF) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Ref 相减。");
    ...
    Value operator * (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 相乘。");
            return Value::makeNumber(numVal * v.numVal);
        } else RUN_ERR("类型 " + typeName[type] + " 不支持 '*'.");
    }
    ...
    Value operator / (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 相除。");
            return Value::makeNumber(numVal / v.numVal);
        } else RUN_ERR("类型 " + typeName[type] + " 不支持 '/'.");
    }
    ...
    Value operator & (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 进行按位与操作。");
    ...
    Value operator xor (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 进行异或操作。");
            return Value::makeNumber((int)numVal xor (int)v.numVal);
        } else RUN_ERR("类型 " + typeName[type] + " 不支持 xor.");
    }

    Value operator | (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 进行按位或操作。");
            return Value::makeNumber((int)numVal | (int)v.numVal);
        } else RUN_ERR("类型 " + typeName[type] + " 不支持 '|'.");
    }
    ...
    Value operator > (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR(" 无法把一个 " + typeName[v.type] + " 和一个 Number 进行比较。");
    ...
    Value operator % (Value & v) {
        if(type == T_NUMBER) {
            if(v.type != T_NUMBER) RUN_ERR("无法把一个 " + typeName[v.type] + " 和一个 Number 进行模运算。");
...
Value getRefVal(Value v, Value locate) {
    ...
    if(v.type != T_STRING && v.type != T_REF) RUN_ERR("无法通过下标访问一个 '" + typeName[v.type] + "'。必须是 String 或 Ref ");
    if(locate.type != T_NUMBER && locate.type != T_STRING) RUN_ERR("无法使用 '" + typeName[v.type] + "' 作为下标。必须是 String 或 实数。" );

    if(locate.type == T_STRING) locate = Value::makeNumber(getHash(locate.strVal));

    if(v.type == T_STRING) {
        int loc = locate.numVal;
        if(loc < 0 || loc >= v.strVal.length()) RUN_ERR("String下标越界。（" + locate.toString() + "）");
        String ret = "";
        ret += v.strVal[loc];
        return Value::makeString(ret);
    } else {
        int position = v.refVal + locate.numVal;
        if(position < 0 || position >= heap.size()) RUN_ERR("非法地址。（" + locate.toString() + " ");
...
void setRefVal(Value v, Value setVal) {
    ...
    if(v.type == T_STRING) RUN_ERR("禁止修改 String 的单个字符。");
    if(v.type != T_REF) RUN_ERR("无法设置 '" + typeName[v.type] + "' 类型的变量。必须是 Ref。");
...
Value makeArray(vector<Value> vals, bool useWay = true) {
    if(vals.size() > ARR_GROW) RUN_ERR("数组长度过长：\n" + parseNum(ARR_GROW) + " < " + parseNum(vals.size()));
...
#define CHECK_TYPE(a, n) \
	if(allParameter[a].type != T_##n) RUN_ERR("内置函数 " + func.funVal + " 需要一个 '" + typeName[T_##n] + "' 作为第 " + parseNum(a) + " 个参数的值类型, 然而给予的: " + typeName[allParameter[a].type])
#define CHECK_ARG_CNT(n)\
    if(allParameter.size() != n) SYNTAX_ERR("调用函数 " + func.funVal + " 的参数个数：" + parseNum(allParameter.size()) + "，然而需要 " + parseNum(n) + "个")
    ...
    if(callStack.size() > INT_MAX) RUN_ERR("调用栈溢出。");
...
String stringExpr(String str) {
    ...
    for(int i = 0; i < str.length(); i++) {
        if(str[i] == '\\') {
            i++;
            if(i > str.length()) RUN_ERR("在 '\\' 之后需要一个字符");
            switch(str[i]) {
                case 'a':
                    result += '\a';
                    break;
...
Value evalExpr(String expr) {
	...
	#define CHECK() if(st.empty()) SYNTAX_ERR("无效的表达式：" + expr) 
    ...
    if(st.size() != 1) SYNTAX_ERR("无效的表达式：" + expr);
...
void setVarRef(String var, Value exp) {
    ...
    ENUM_VS(protectVar, i) {
    	if(var == protectVar[i]) RUN_ERR(" 拒绝访问：" + var + " 是一个只读变量.");
	}
...
VS makeBlock() {
    ...
    while(getline(codeStream, s)) {
        ...
        if(vs[0] == "if") {
            ...
            if(condition.size() != 7) SYNTAX_ERR("for语句的正确格式：for('name', expression, expression, expression) {block}");
...
void readByteCode(String bytefile) {
    ...
    getline(fcin, temp);
    if(temp != "ff0") {
        RUN_ERR("这不是一个有效的 .ff0 文件： " + bytefile + "。请检查您的文件是否被破坏。");
    }
    getline(fcin, temp);
...
String javascript() {
    ...
    for(map<String, VS>::iterator it = funcTable.begin(); it != funcTable.end(); it++) {
        ...
        if(!uncomped[fun] && fun[0] != '~') {
        	jscout << "function " << fun << " ( ";
			for(int i = 0; i < paramIt -> second.size(); i++) jscout << paramIt -> second[i] << ",";
			jscout << "\b) {" << endl;
...
int main(int argc, char ** argv) {
    ...
    if(argc == 1) {
        ...
        codefileName = "a.ff0";
        try {
        	registFunc("<main>", EMPTY, makeBlock());
		}

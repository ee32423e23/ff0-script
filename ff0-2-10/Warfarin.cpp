...
const string VERSION = "Warfarin script 2.10";
...
Value makeArray(vector<Value> vals) {
    ...
    usedMemory[usedMemory.size() - 1].push_back(icIndex);
...
VS tokenize(String text) {
	...
	for(int i = 0; i < text.length(); i++) {
		if(text[i] == '(') {
			...
			while(top && i < text.length()) {
				i++;
				if(!isspace(text[i])) cur = cur + text[i];
				if(text[i] == '(') top++;
				else if(text[i] == ')') top--;
			}
			tmp.push_back(cur);
			...
			cur = "\"";
			while(i < text.length() && i < text.length()) {
				i++;
				if(text[i] == '\\') {
					cur = cur + text[i];
	...
	ENUM_VS(tmp, i) {
		if(tmp[i] != "") {
			DEBUG_OUTPUT << "ret[" << i << "] = " << tmp[i] << " length = " << tmp[i].length() << endl;
			ret.add(trim(tmp[i]));
			DEBUG_OUTPUT << "trimed = " << trim(tmp[i]) << " length = " << trim(tmp[i]).length() << endl;
		}
	}
    ENUM_VS(ret, i) {
        if(i > 0) {

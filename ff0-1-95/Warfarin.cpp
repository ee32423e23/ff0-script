...
const string VERSION = "Warfarin script 1.95";
...
String freadString(int handle) {
	...
	String result = "";
	char c;
	while(isspace(c)) c = fgetc(filePtr[handle]);
...
#define CHECK_ARG_CNT(n)\
    ...
    } else if(func.toString() == "readonly") {
    ...
    for(int i = 0; i < vs.size() - 1; i++) {
        ...
        if(vs[i] == "@") {
            ...
            */
            ...
            if(vs[i][0] == '(') {
                simpleExpr = MIDDLE(vs[i]);
                DEBUG_OUTPUT << "() ed simple value = " << simpleExpr << endl;
                num = evalExpr(simpleExpr);
            } else if(vs[i][0] == '[') {
                simpleExpr = MIDDLE(vs[i]);
                num = Value::makeString(simpleExpr);
            	...
            	num = evalSimpleExpr(simpleExpr);
            	DEBUG_OUTPUT << "simpleExpr = " << simpleExpr << " = " << num.toString() << endl; 
			}
...
Value execBlock(VS block) {
    ENUM_VS(block, index) {
        ...
        if(token[0] == "if") {
            ...
            while((!evalExpr(condition)).type == T_FALSE) callFunc(Value::makeFunction(token[2]), EMPTY_V, false);
        } else if(token[0] == "for") {
